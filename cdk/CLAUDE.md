...
|------|------|---------------------|
| `bin/app.ts` | CDK app entry point | `main.tf` (root) |
| `lib/rum-pipeline-stack.ts` | Main stack — composes all Constructs | `main.tf` |
| `lib/constructs/s3-data-lake.ts` | S3 buckets + lifecycle | `modules/s3-data-lake/` |
| `lib/constructs/glue-catalog.ts` | Glue DB + 3 tables | `modules/glue-catalog/` |
| `lib/constructs/firehose.ts` | Firehose + Transform Lambda | `modules/firehose/` |
| `lib/constructs/security.ts` | WAF + SSM + Authorizer | `modules/security/` |
| `lib/constructs/api-gateway.ts` | HTTP API + Ingest Lambda | `modules/api-gateway/` |
| `lib/constructs/grafana.ts` | Managed Grafana + Athena WG | `modules/grafana/` |
| `lib/constructs/monitoring.ts` | CloudWatch Dashboard | `modules/monitoring/` |
| `lib/constructs/partition-repair.ts` | Partition repair Lambda + EventBridge | `modules/partition-repair/` |
| `lib/constructs/athena-query.ts` | Athena Query Lambda | `modules/athena-query/` |
| `lib/constructs/agent-ui.ts` | CloudFront + ALB + EC2 | `modules/agent-ui/` |
| `lib/constructs/auth.ts` | Cognito + SSO + Lambda@Edge | `modules/auth/` |
...
```bash
cd cdk && npm install
cd cdk && npx cdk synth          # Generate CloudFormation template
cd cdk && npx cdk diff           # Review changes
cd cdk && npx cdk deploy         # Deploy
cd cdk && npx cdk destroy        # Destroy
```
...
### Rules
- Lambda source references the `../lambda/` directory directly
- Follows the same resource naming convention as Terraform (`{projectName}-{component}`)
- Security-sensitive values passed via context or environment variables (no hardcoding)
- Default values can be set in `cdk.json` context
